...
// ─────────────────────────────────────────────────────────────
// 8. SUPPORT — New ticket (to admin) + Reply (to user)
// ─────────────────────────────────────────────────────────────
export async function sendSupportTicketReceived(opts: {
  to: string;
  subject: string;
  category: string;
  ticketId: number;
}) {
  const { to, subject, category, ticketId } = opts;
  const body = `
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td style="padding-bottom:24px;text-align:center;">
          ...
          <h1 style="margin:0 0 4px;font-size:20px;font-weight:800;color:${BRAND.text};">Ticket recebido!</h1>
          <p style="margin:0;font-size:14px;color:${BRAND.muted};">A nossa equipa irá responder em breve.</p>
        </td>
      </tr>
      ${divider()}
      <tr><td style="padding:16px 0;">
        <table width="100%" cellpadding="0" cellspacing="0">
          ...
          ${row("ID do ticket", `#${ticketId}`, BRAND.muted)}
          ${row("Status", "Aberto", BRAND.amber)}
        </table>
      </td></tr>
      ${divider()}
      <tr><td style="padding-top:16px;font-size:13px;color:${BRAND.muted};line-height:1.6;">
        Responderemos ao seu pedido de suporte dentro de 1–2 dias úteis. Receberá uma notificação por email quando respondermos.
      </td></tr>
    </table>`;
  return resend.emails.send({
    from: FROM,
    to,
    subject: `🎫 Ticket de suporte recebido — #${ticketId}`,
    html: baseLayout("Ticket de Suporte", body),
  });
}
...
export async function sendSupportTicketReply(opts: {
  ...
  const { to, subject, reply, ticketId } = opts;
  const body = `
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td style="padding-bottom:24px;text-align:center;">
          ...
          <h1 style="margin:0 0 4px;font-size:20px;font-weight:800;color:${BRAND.text};">Resposta ao seu ticket</h1>
          <p style="margin:0;font-size:14px;color:${BRAND.muted};">A equipa Meteorfy respondeu ao seu pedido.</p>
        </td>
      </tr>
      ${divider()}
      <tr><td style="padding:16px 0;">
      ...
      ${divider()}
      <tr><td style="padding-top:16px;">
        <p style="margin:0 0 8px;font-size:11px;font-weight:700;color:${BRAND.muted};text-transform:uppercase;letter-spacing:1px;">Resposta da equipa</p>
        <div style="background:#27272a;border-left:3px solid ${BRAND.purple};border-radius:8px;padding:14px 16px;">
          <p style="margin:0;font-size:13px;color:${BRAND.text};line-height:1.7;white-space:pre-line;">${reply}</p>
        </div>
      </td></tr>
    </table>`;
  return resend.emails.send({
    from: FROM,
    to,
    subject: `💬 Resposta ao seu ticket #${ticketId} — ${subject}`,
    html: baseLayout("Resposta do Suporte", body),
  });
}

export async function sendAdminNewTicket(opts: {
  subject: string;
  category: string;
  message: string;
  userEmail: string;
  ...
  const { subject, category, message, userEmail, ticketId } = opts;
  const adminUrl = `https://${process.env.REPLIT_DOMAINS?.split(",")[0] || "meteorfy.com"}/admin/support`;
  const body = `
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td style="padding-bottom:24px;text-align:center;">
          <div style="display:inline-block;padding:10px 20px;background:#ef444422;border:1px solid #ef444444;border-radius:100px;margin-bottom:16px;">
            <span style="font-size:12px;font-weight:700;color:#ef4444;letter-spacing:1px;text-transform:uppercase;">🎫 Novo Ticket</span>
          </div>
          <h1 style="margin:0 0 4px;font-size:20px;font-weight:800;color:${BRAND.text};">Novo pedido de suporte</h1>
        </td>
      </tr>
      ${divider()}
      <tr><td style="padding:16px 0;">
        <table width="100%" cellpadding="0" cellspacing="0">
          ${row("Utilizador", userEmail)}
          ${row("Assunto", subject)}
          ${row("Categoria", category)}
          ${row("ID", `#${ticketId}`, BRAND.muted)}
        </table>
      </td></tr>
      ${divider()}
      <tr><td style="padding-top:16px;">
        <p style="margin:0 0 8px;font-size:11px;font-weight:700;color:${BRAND.muted};text-transform:uppercase;letter-spacing:1px;">Mensagem</p>
        <div style="background:#27272a;border-radius:8px;padding:14px 16px;">
          <p style="margin:0;font-size:13px;color:${BRAND.text};line-height:1.7;white-space:pre-line;">${message}</p>
        </div>
      </td></tr>
      ${divider()}
      <tr><td style="padding-top:20px;text-align:center;">
        <a href="${adminUrl}" style="display:inline-block;background:#ef4444;color:#fff;text-decoration:none;font-weight:700;font-size:13px;padding:12px 28px;border-radius:10px;">
          Responder no Admin →
        </a>
      </td></tr>
    </table>`;
  return resend.emails.send({
    from: FROM,
    to: "yuldchissico11@gmail.com",
    subject: `🎫 Novo ticket #${ticketId} — ${subject}`,
    html: baseLayout("Novo Ticket de Suporte", body),
  });
}

// ─────────────────────────────────────────────────────────────
// 9. NEW USER — Welcome + Email Verification reminder
// ─────────────────────────────────────────────────────────────
