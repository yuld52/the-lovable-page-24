...
function getSoftBackgroundColor(color: string): string {
    ...
    const newS = Math.min(s * 0.35, 0.25), newL = 0.94;
    ...
    const p = 2 * newL - q;
    const newR = hue2rgb(p, q, h + 1 / 3), newG = hue2rgb(p, q, h), newB = hue2rgb(p, q, h - 1 / 3);
...
export default function PublicCheckout() {
    ...
    const { data: product, isLoading: isLoadingProduct } = useQuery<Product | null>({
        ...
        queryFn: async () => {
            ...
            return { id: docSnap.id, name: data.name, price: data.price, imageUrl: data.image_url, deliveryUrl: data.delivery_url } as any;
        },
    });
                            ...
                            <div className="p-4 space-y-4">
                                {orderBumpProductsData.map(p => (
                                    <div key={p.id} className="rounded-lg border-2 border-dashed" style={{ borderColor: config.primaryColor }}>
                                        ...
                                        <div className="flex items-center gap-2 p-3 cursor-pointer" style={{ backgroundColor: getSoftBackgroundColor(config.primaryColor) }} onClick={() => orderBumpSelected.includes(p.id) ? setOrderBumpSelected(orderBumpSelected.filter(id => id !== p.id)) : setOrderBumpSelected([...orderBumpSelected, p.id])}>
