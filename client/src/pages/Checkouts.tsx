    ...
    <Layout title="Checkouts" subtitle="Páginas de venda personalizadas">
      ...
      {isLoading && checkouts === undefined ? (
        ...
        <Card className="bg-card border-border/60 flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-medium text-foreground mb-1">Nenhum checkout</h3>
          ...
          <Button
            className="bg-zinc-800 hover:bg-zinc-700 text-white border border-border"
            onClick={handleCreate}
          >
        ...
        <div className="flex flex-col gap-2">
          {checkouts?.map((checkout) => (
            <Card key={checkout.id} className="bg-card border-border/60 hover:border-purple-500/30 transition-all cursor-pointer group overflow-hidden w-full flex flex-row items-center px-3 py-2.5 gap-3">
              <div className="w-10 h-10 bg-muted rounded-lg flex-shrink-0 flex items-center justify-center border border-border/50">
                <ShoppingCart className="w-4 h-4 text-muted-foreground" />
              </div>

              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-semibold text-foreground group-hover:text-purple-400 transition-colors truncate">
                  {checkout.name}
                </h3>
