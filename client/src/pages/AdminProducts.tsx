...
function DetailRow({ icon: Icon, label, value }: { icon: any; label: string; value: React.ReactNode }) {
  return (
    <div className="flex gap-3">
      <div className="w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-lg bg-zinc-800 border border-border">
        <Icon className="w-4 h-4 text-muted-foreground" />
      </div>
      <div className="min-w-0">
        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-0.5">{label}</p>
        <div className="text-sm text-zinc-200 break-all">{value || <span className="text-muted-foreground italic">Não informado</span>}</div>
      </div>
    </div>
  );
}
        ...
        <div className="space-y-6 pb-2">
          ...
          <div className="flex items-center justify-between">
            ...
            {product.status === "pending" && (
              <div className="flex gap-2">
                <Button
                  size="sm"
                  className="bg-emerald-600 hover:bg-emerald-500 text-foreground h-8 gap-1.5"
          ...
          <div className="space-y-4">
            ...
            <DetailRow
              ...
              value={
                product.description ? (
                  <p className="whitespace-pre-wrap text-foreground/80 leading-relaxed">{product.description}</p>
                ) : null
              }
            />
            <DetailRow
                ...
                deliveryFiles.length > 0 ? (
                  <ul className="space-y-1 mt-1">
                    {deliveryFiles.map((f: any, i: number) => (
                      <li key={i} className="flex items-center gap-2 text-xs bg-muted rounded px-3 py-2 border border-border">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {paymentMethods.map((m) => (
                <span key={m} className="flex items-center gap-1.5 bg-zinc-800 border border-border text-foreground/80 px-3 py-1.5 rounded-lg text-xs font-medium">
          ...
          {product.status === "pending" && (
            <>
              ...
              <div className="flex justify-end gap-2">
                <Button
                  className="bg-emerald-600 hover:bg-emerald-500 text-foreground gap-1.5"
                  onClick={() => onApprove(product.id)}
                  disabled={approving || rejecting}
                >
...
export default function AdminProducts() {
  ...
  const tabs = [
    ...
    { key: "all", label: "Todos", color: "text-foreground/80 border-zinc-500" },
  ] as const;
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-foreground">Aprovação de Produtos</h1>
          ...
          <div className="flex gap-1 mb-6 bg-muted/50 p-1 rounded-xl border border-border/60 w-fit">
            {tabs.map((tab) => (
              ...
              >
                {tab.label}
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-muted ${activeTab === tab.key ? tab.color : "text-muted-foreground"}`}>
                  {counts[tab.key]}
                </span>
              </button>
            ))}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-muted/80 border-b border-border/50">
                        <th className="px-6 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Produto</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Preço</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Entrega</th>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <Button
                                size="sm"
                                variant="outline"
                                className="border-border text-foreground/80 hover:bg-accent h-8 gap-1.5"
                              ...
                              {(p.status || "pending") === "pending" && (
                                <>
                                  <Button
                                    size="sm"
                                    className="bg-emerald-600 hover:bg-emerald-500 text-foreground h-8"
