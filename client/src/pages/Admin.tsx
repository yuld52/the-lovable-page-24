        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Páginas de venda</p>
                </CardContent>
                </Card>
                ...
                <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Soma de todas as vendas pagas</p>
                </CardContent>
                </Card>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Ranking por número de vendas aprovadas</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-0 mt-2">
                {topProducts.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground text-sm">Nenhuma venda aprovada ainda</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-muted/80 border-y border-border/50">
                          <th className="px-4 md:px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase tracking-wider w-12">#</th>
                          <th className="px-4 md:px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Produto</th>
                          <th className="px-4 md:px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase tracking-wider text-center">Vendas</th>
                          <th className="px-4 md:px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase tracking-wider text-right">Receita</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border/30">
                        {topProducts.map((p) => (
                          <tr key={p.productId} className={`hover:bg-accent/20 transition-colors ${p.rank === 1 ? 'bg-orange-500/5' : p.rank === 2 ? 'bg-zinc-400/5' : p.rank === 3 ? 'bg-amber-700/5' : ''}`}>
                            <td className="px-4 md:px-6 py-3">
                              {p.rank === 1 ? (
                                ...
                                <div className="flex items-center justify-center w-8 h-8 rounded-full bg-zinc-400/20 border border-zinc-400/30">
                                  <Medal className="w-3.5 h-3.5 text-foreground/80" />
                                </div>
                              ) : p.rank === 3 ? (
                                ...
                                <div className="flex items-center justify-center w-8 h-8 rounded-full bg-zinc-800 border border-border">
                            ...
                            <td className="px-4 md:px-6 py-3">
                              <div className="flex items-center gap-3">
                                {p.imageUrl ? (
                                  ...
                                  <div className="w-9 h-9 rounded-lg bg-zinc-800 border border-border flex items-center justify-center flex-shrink-0">
                            ...
                            <td className="px-4 md:px-6 py-3 text-center">
                              <span className={`inline-flex items-center gap-1 text-sm font-bold px-2 py-0.5 rounded-full ${p.rank === 1 ? 'bg-orange-500/10 text-orange-400' : 'bg-zinc-800 text-foreground/80'}`}>
                                {p.salesCount}
                              </span>
                            </td>
